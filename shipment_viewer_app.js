...
const now=new Date();
const API_BASE_URL=(WORKER_BASE_URL||'').replace(/\/$/,'');
...
const compareDateTime=(a,b)=>{
  const da=(a.shipDate||'9999-99-99').trim()||'9999-99-99',db=(b.shipDate||'9999-99-99').trim()||'9999-99-99';
  if(da!==db) return da.localeCompare(db);
  const ta=(a.loadTime||'').trim()||'99:99',tb=(b.loadTime||'').trim()||'99:99';
...
async function apiRequest(path,options,allow401){
  const init=Object.assign({credentials:'include',cache:'no-store',mode:'cors',headers:{}},options||{});
  ...
  if(!res.ok){
    const msg=(payload&&payload.error)||(payload&&payload.message)||(`${res.status} ${res.statusText}`);
...
async function onLogout(){
  setBusy(true);
  try{ await apiRequest(ENDPOINTS.logout,{method:'POST'},true); }catch(_){}
  currentUser=null;
  appData=[];
...
function getErrMsg(err){
  return (err&&err.message)?err.message:'오류';
}
...
const renderViewTabs=()=>{refs.viewTabs.innerHTML=viewOptions.map(o=>`<button class="tab ${state.view===o.value?'active':''}" data-v="${o.value}" type="button">${esc(o.label)}</button>`).join('');refs.viewTabs.querySelectorAll('.tab').forEach(btn=>btn.addEventListener('click',()=>{state.view=btn.dataset.v;renderAll();}));};
...
function renderItemDetail(item){
  const isEtc=item.category==='기타';
  return `<div class="detail-grid">`+field('법인',item.corp)+field('구분',item.shipMode)+field('출발지',item.depart)+field('시트 구분',item.shipType)+field('출하일',item.shipDate)+field('상차시간',item.loadTime)+field('하차시간',item.unloadTime)+field('FAB OUT',item.fabOut)+field('포장예정',item.packaging)+field('사이즈 전달',item.sizeNotice)+field('출하의뢰',item.shipRequest)+field('통관요청',item.customs)+field('TERMS',item.terms)+field('PSK배차',item.pskDelivery)+field('포워더',item.forwarder)+field('W/O',item.wo)+field('S/N',item.sn)+field('장비명',item.model,true)+field('고객사',item.customer,true)+(isEtc?field('업체명',item.vendorName)+field('부분품',item.moduleText,true):'')+field('담당자',item.manager)+field('INV',item.inv)+field('수출여부 원문',item.exportInfo,true)+`</div>`;
}
function renderList(items){
  if(!items.length){state.listDate='';refs.listEl.innerHTML='<div class="empty">조건에 맞는 데이터가 없습니다.</div>';return;}
  const dates=listDateCandidates(items);
  ...
  const prevBtn=$('listPrevDate');const nextBtn=$('listNextDate');
...
function renderCalendar(items){
  const y=state.calendarYear,m=state.calendarMonth,last=new Date(y,m+1,0).getDate();
  const byDate=items.reduce((a,item)=>{if(item.shipDate)(a[item.shipDate]=a[item.shipDate]||[]).push(item);return a;},{});
  const pad=n=>String(n).padStart(2,'0');
  let html='',week=[];
  for(let day=1;day<=last;day++){
    const ds=`${y}-${pad(m+1)}-${pad(day)}`,wd=new Date(ds+'T00:00:00').getDay();
...
function renderCalendarDayPanel(items){
  if(!state.selectedDate){refs.calDayPanel.innerHTML='<div class="day-panel-title">날짜를 선택하세요</div>';return;}
  const rows=sortByLoadTime(items.filter(item=>item.shipDate===state.selectedDate));
  if(!rows.length){refs.calDayPanel.innerHTML=`<div class="day-panel-title">${esc(state.selectedDate)}</div><div class="empty">선택한 날짜의 데이터가 없습니다.</div>`;return;}
  refs.calDayPanel.innerHTML=`<div class="day-panel-title">${esc(state.selectedDate)} · ${rows.length}건</div><div class="day-list">${rows.map((item,i)=>{const workLoc=item.depart==='조립업체'?(item.vendorName||item.depart):((item.category==='기타')?(item.locationText||item.depart):item.depart);return `<div class="day-card ${item.redAlert==='Y'?'alert':''}" data-idx="${i}"><div class="day-card-title">${esc(item.wo||'-')} | ${esc(item.model||'-')}</div><div class="day-card-sub">${esc(item.customer||'-')} · ${esc(item.corp||'-')} · ${esc(item.shipMode||'-')}</div><div class="day-card-sub">작업지: ${esc(workLoc||'-')} · 상차시간: ${esc(item.loadTime||'-')}</div></div>`;}).join('')}</div>`;
...
function buildReportTable(corp,rows){
  ...
  const dateCounts={};
  rows.forEach(item=>{const k=String(item[dateKey]||'');dateCounts[k]=(dateCounts[k]||0)+1;});
  const datePrinted={};
  let body='';
  rows.forEach(item=>{
...
function renderReport(items){
  if(!items.length){refs.reportEl.innerHTML='<div class="empty">조건에 맞는 데이터가 없습니다.</div>';return;}
  ...
  refs.reportEl.innerHTML=reportCorpOrder.filter(c=>corpGroups[c]&&corpGroups[c].length).map(corp=>{
    const deptGroups=corpGroups[corp].reduce((a,item)=>{const dep=((item.depart||'').trim())||'기타';(a[dep]=a[dep]||[]).push(item);return a;},{});
...
function renderMainView(items){
  const isCalendar=state.view==='calendar',isReport=state.view==='report';
...
function bindStaticEvents(){
  ...
  refs.dateResetEl.addEventListener('click',()=>{state.dateFrom='';state.dateTo='';refs.dateFromEl.value='';refs.dateToEl.value='';renderAll();});
  refs.toggleAdvancedFiltersBtn.addEventListener('click',()=>{state.advancedOpen=!state.advancedOpen;renderAll();});
  $('prevMonth').addEventListener('click',()=>{state.calendarMonth--;if(state.calendarMonth<0){state.calendarMonth=11;state.calendarYear--;}state.selectedDate='';renderAll();});
  $('nextMonth').addEventListener('click',()=>{state.calendarMonth++;if(state.calendarMonth>11){state.calendarMonth=0;state.calendarYear++;}state.selectedDate='';renderAll();});
  refs.modalBackdrop.addEventListener('click',closeDetailModal);
  refs.modalClose.addEventListener('click',closeDetailModal);
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!refs.detailModal.classList.contains('hidden'))closeDetailModal();});
}
